...
public class UniversalBlurFeature : ScriptableRendererFeature {
	...
	public override void AddRenderPasses(ScriptableRenderer renderer, ref RenderingData renderingData) {
		if (passMaterial == null) {
			Debug.LogWarningFormat(
				"Missing Post Processing effect Material. {0} Fullscreen pass will not execute. Check for missing reference in the assigned renderer.",
				GetType().Name);
			return;
		}
	...
	void SetupPassData(UniversalBlurPass.PassData passData) {
		...
		passData.profilingSampler     ??= new UnityEngine.Rendering.ProfilingSampler("FullScreenPassRendererFeature");
